...
export const CartProvider = ({children}) => {
    ...
    const addProduct = (item, quantity) => {
        ...
        if (!productExistente) {
            ...
            setTotal(prev => Math.round((prev + (item.price * quantity)) * 100) / 100);
        } else {
    ...
    const removeProduct = (id) => {
        ...
        setTotal(prev => Math.round((prev - (productEliminado.item.price * productEliminado.quantity)) * 100) / 100);
    }
    ...
    return (
        <CartContext.Provider value={{cart, total, quantityTotal, addProduct, removeProduct, emptyCart}}>
            {children}
